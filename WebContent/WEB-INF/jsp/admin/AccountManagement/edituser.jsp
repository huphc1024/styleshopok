	...
	<div class="container-fluid">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form action="${pageContext.request.contextPath }/admincp/account/user/edit/${objUser.id_user }" method="post" enctype="multipart/form-data"
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select name="active" id="input-status" class="form-control">
								<c:choose>
									<c:when test="${not empty objUser.role }">
										<c:if test="${1 == objUser.active }">
											<c:set value="selected='selected'" var="bat"></c:set>
										</c:if>
										<c:if test="${0 == objUser.active }">
											<c:set value="selected='selected'" var="tat"></c:set>
										</c:if>
									</c:when>
									<c:otherwise>
										<c:set value="selected='selected'" var="activenull"></c:set>
									</c:otherwise>
								</c:choose>
								<option ${tat } value="0">Tắt</option>
								<option ${bat } value="1">Bật</option>
							</select>
						</div>
					</div>
				</form>
